...
the v4 cell-centroid PCA basis, measure distance-to-MR-centroid as a
...
5. Outputs:
   - 3D HTML plot per arm, showing centroids as big markers + each
     ...
     index).
   - 2D PNG: mean distance-to-MR-centroid vs token index, per arm,
     with shaded ±1σ band.
...
def _build_centroids(
    ...
    if not lb_jsonl.exists():
        print(f"  no bliss-content pilot at {lb_jsonl}; "
              f"MR centroid will be missing")
    else:
        df_raw_lb, X3_lb_raw, layers_lb = _load_and_filter_3d(
            lb_jsonl, lb_experiment,
        )
        # Post-2026-05-11: apply_pad_split inside _load_and_filter_3d
        # already canonicalizes LB → MR, so we filter on "MR" here
        # rather than on the pre-rename "LB" label.
        mr_mask = (df_raw_lb["quadrant"] == "MR").to_numpy()
        df_lb = df_raw_lb.loc[mr_mask].reset_index(drop=True)
        X3_lb = X3_lb_raw[mr_mask]
        print(f"  bliss-content pilot (MR cell): {len(df_lb)} rows "
              f"({len(layers_lb)} layers)")

    # Triple intersection: main ∩ bliss-content ∩ attractor.
    common = sorted(set(layers_main) & set(attractor_layers))
    if layers_lb:
        common = sorted(set(common) & set(layers_lb))
    if not common:
        raise SystemExit("no common layers across main / bliss / attractor")
    print(f"  layer intersection: {len(common)} layers "
          f"(main={len(layers_main)}, bliss={len(layers_lb) or 'n/a'}, "
    ...
    if df_lb is not None and X3_lb is not None and len(df_lb) > 0:
        ...
        centroids["MR"] = X3_lb_c.mean(axis=0).reshape(nL * hD)
...
def _plot_arm_3d(
    ...
    *,
    ...
    # Centroid markers (sized by importance to the basin question: MR
    # gets a bigger marker since it's the basin under study). Color
    # uses QUADRANT_COLORS so the plot reads consistent with the rest
    # of the figure surface.
...
def _plot_distance_curves(
    ...
    *,
    ...
    """Mean distance-to-MR-centroid vs token index per arm, with ±1σ
    ...
    if "MR" not in centroids:
        print("  no MR centroid; skipping distance plot")
        return
    mr = centroids["MR"]
    ...
    for arm, rows in arm_to_rows.items():
        ...
        for i, r in enumerate(rows):
            d = _distance_to_centroid(r["trajectory"], mr)
            D[i, : d.shape[0]] = d
    ...
    ax.set_title("distance to MR centroid over token index, per arm")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            "Optional short name of a different model to source centroids "
            "from. When set, trajectories load from --model but centroids "
            "load from --reference. Used for the base-vs-instruct "
            "comparison: e.g. --model gemma_base --reference gemma "
            "projects base-model trajectories into instruct-model "
            "centroid space to test whether the basin is pretraining-"
            "anchored. The two models must share architecture / "
            "hidden_dim; layer sets are auto-intersected."
        ),
    )
    args = ap.parse_args()
    short = args.model
    ...
    if reference not in MODEL_REGISTRY:
        raise SystemExit(f"unknown reference model {reference!r}")
    if reference != short:
        print(f"reference model for centroids: {reference} "
              f"(trajectories from {short})")
